...
export class SubagentOverlayComponent implements Component, Focusable {
  ...
  private get rows(): RunRow[] { return projectConversations(this.manager.listConversations()).flatMap(({conversation})=>conversation.runs.map(run=>({conversation,run}))); }
  ...
  private canResume(row:RunRow): boolean { return row.conversation.canResume && row.conversation.runs.at(-1)?.runId===row.run.runId && row.run.status.kind==="done"; }
