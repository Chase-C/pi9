...
| `list` | Return immediate child conversations by default, or the caller's full owned subtree with `scope: "descendants"`, without run output. It is pure: it acknowledges nothing and changes no lifecycle state. |
...
`list` defaults to immediate children. Pass `scope: "descendants"` for the complete owned subtree; at the root this is the global runtime inventory. Listed conversations expose conversation-level parent, spawn provenance, and depth, while their runs remain parentless episodes. Inspection is pure and bounded; malformed, duplicate, unknown, or unauthorized targets return ordered per-target errors without hiding valid siblings. A running snapshot includes `phase`, requested overrides, effective configuration, recent activity, and steer receipts. Terminal output remains exclusive to `join`.
